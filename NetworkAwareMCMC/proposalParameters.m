...
hessianThreshold=1E-12;     % hessian lower bound
...
if isempty(deleteReactions) && numberOfVariables~=0
    ...
    if optimizedParameters==1
        ...
        if proposalType==1
                ...
                [centeringLocation]=fminunc(@(centeringLocation)optimizationObjective(centeringLocation,addReactions,RJMCMCType,key,keyCount,intermediateCurrentInfluentialReactions,intermediateProposedInfluentialReactions,N,valArray,val,leadLogLikelihood,logPos,pos,priorRange,input,data,delta,dataVariance,nReactions,nSpecies,modelProblem,initialIncludedSpeciesPool,targetNode,reactionType,speciesReference,stoichiometricMatrix,typeSampling,numForwardModelSolves,shouldDisplay,proposalType,meanVector,valVector),x0,opt);
            ...
            if derivativeBased==1
                ...
                [centeringLocation,fval,~,~,lambda,grad,H]=fmincon(@(centeringLocation)optimizationObjective(centeringLocation,addReactions,RJMCMCType,key,keyCount,intermediateCurrentInfluentialReactions,intermediateProposedInfluentialReactions,N,valArray,val,leadLogLikelihood,logPos,pos,priorRange,input,data,delta,dataVariance,nReactions,nSpecies,modelProblem,initialIncludedSpeciesPool,targetNode,reactionType,speciesReference,stoichiometricMatrix,typeSampling,numForwardModelSolves,shouldDisplay,proposalType,meanVector,valVector),x0,[],[],[],[],lb,ub,[],opt);
        ...
        if proposalType==1
            ...
            if selfCode==1
                ...
                [hessianDERIVEST,err] = hessian(@(x)optimizationObjective(x,addReactions,RJMCMCType,key,keyCount,intermediateCurrentInfluentialReactions,intermediateProposedInfluentialReactions,N,valArray,val,leadLogLikelihood,logPos,pos,priorRange,input,data,delta,dataVariance,nReactions,nSpecies,modelProblem,initialIncludedSpeciesPool,targetNode,reactionType,speciesReference,stoichiometricMatrix,typeSampling,numForwardModelSolves,shouldDisplay,proposalType,meanVector,valVector),centeringLocation);
                
                
                [gradientDERIVEST,err] = gradest(@(x)optimizationObjective(x,addReactions,RJMCMCType,key,keyCount,intermediateCurrentInfluentialReactions,intermediateProposedInfluentialReactions,N,valArray,val,leadLogLikelihood,logPos,pos,priorRange,input,data,delta,dataVariance,nReactions,nSpecies,modelProblem,initialIncludedSpeciesPool,targetNode,reactionType,speciesReference,stoichiometricMatrix,typeSampling,numForwardModelSolves,shouldDisplay,proposalType,meanVector,valVector),centeringLocation);
...
elseif isempty(addReactions) && numberOfVariables~=0
    ...
    if optimizedParameters==1
        ...
        if proposalType==1
                ...
                [centeringLocation]=fminunc(@(centeringLocation)optimizationObjective(centeringLocation,deleteReactions,RJMCMCType,key,keyCount,intermediateCurrentInfluentialReactions,intermediateProposedInfluentialReactions,N,valArray,val,leadLogLikelihood,logPos,pos,priorRange,input,data,delta,dataVariance,nReactions,nSpecies,modelProblem,initialIncludedSpeciesPool,targetNode,reactionType,speciesReference,stoichiometricMatrix,typeSampling,numForwardModelSolves,shouldDisplay,proposalType,meanVector,valVector),x0,opt);
            ...
            if derivativeBased==1
                ...
                [centeringLocation,fval,~,~,lambda,grad,H]=fmincon(@(centeringLocation)optimizationObjective(centeringLocation,deleteReactions,RJMCMCType,key,keyCount,intermediateCurrentInfluentialReactions,intermediateProposedInfluentialReactions,N,valArray,val,leadLogLikelihood,logPos,pos,priorRange,input,data,delta,dataVariance,nReactions,nSpecies,modelProblem,initialIncludedSpeciesPool,targetNode,reactionType,speciesReference,stoichiometricMatrix,typeSampling,numForwardModelSolves,shouldDisplay,proposalType,meanVector,valVector),x0,[],[],[],[],lb,ub,[],opt);
                ...
                opt.lower_bounds = lb;
                opt.upper_bounds = ub;
                opt.min_objective = (@(centeringLocation)optimizationObjective(centeringLocation,deleteReactions,RJMCMCType,key,keyCount,intermediateCurrentInfluentialReactions,intermediateProposedInfluentialReactions,N,valArray,val,leadLogLikelihood,logPos,pos,priorRange,input,data,delta,dataVariance,nReactions,nSpecies,modelProblem,initialIncludedSpeciesPool,targetNode,reactionType,speciesReference,stoichiometricMatrix,typeSampling,numForwardModelSolves,shouldDisplay,proposalType,meanVector,valVector));
                opt.fc_tol = 1e-5*ones(1,length(lb));
                opt.xtol_rel = 1e-6;
                opt.ftol_rel = 1e-6;
                %opt.verbose=1;
        ...
        if proposalType==3
         ...
         if proposalType==1
             ...
             if selfCode==1
                 ...
                 [hessianDERIVEST,err] = hessian(@(x)optimizationObjective(x,deleteReactions,RJMCMCType,key,keyCount,intermediateCurrentInfluentialReactions,intermediateProposedInfluentialReactions,N,valArray,val,leadLogLikelihood,logPos,pos,priorRange,input,data,delta,dataVariance,nReactions,nSpecies,modelProblem,initialIncludedSpeciesPool,targetNode,reactionType,speciesReference,stoichiometricMatrix,typeSampling,numForwardModelSolves,shouldDisplay,proposalType,meanVector,valVector),centeringLocation);
                 
                 [gradientDERIVEST,err] = gradest(@(x)optimizationObjective(x,deleteReactions,RJMCMCType,key,keyCount,intermediateCurrentInfluentialReactions,intermediateProposedInfluentialReactions,N,valArray,val,leadLogLikelihood,logPos,pos,priorRange,input,data,delta,dataVariance,nReactions,nSpecies,modelProblem,initialIncludedSpeciesPool,targetNode,reactionType,speciesReference,stoichiometricMatrix,typeSampling,numForwardModelSolves,shouldDisplay,proposalType,meanVector,valVector),centeringLocation);
